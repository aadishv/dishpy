...
class Prefixer(ast.NodeTransformer):
    ...
    def visit_Name(self, node):
        ...
        if original_name in self.declarations and isinstance(node.ctx, (ast.Store, ast.Del)):
    ...
    for root, _, files in os.walk(project_dir):
        ...
        for file in files:
            if file.endswith('.py'):
                ...
                module_name = os.path.relpath(file_path, project_dir).replace(os.sep, '.')
                if file == '__init__.py':
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if alias.name == 'vex' or alias.name.startswith('vex.'):
                        if verbose:
                            print(f"DEBUG: '{alias.name}' is treated as external (vex library)")
                        ...
                        if verbose:
                            print(f"DEBUG: '{alias.name}' is local module at {dep_path}")
                ...
                if module_name == 'vex' or (module_name and module_name.startswith('vex.')):
                    if verbose:
                        print(f"DEBUG: '{module_name}' is treated as external (vex library)")
                    external_imports.add(ast.unparse(node))
                else:
                    ...
                    if is_local and origin_file:
                        ...
                        for alias in node.names:
                            if alias.name == '*':
                                ...
                                if verbose:
                                    print(f"DEBUG: Wildcard import from {os.path.basename(origin_file)} in {os.path.basename(current_file)}")
                                ...
                                if verbose:
                                    print(f"DEBUG: Symbol '{alias.name}' in {os.path.basename(current_file)} comes from {os.path.basename(origin_file)}")
    ...
    for file_path, origins in symbol_origins.items():
        if '__WILDCARD_FROM__' in origins:
            ...
            for symbol in declared_symbols.get(wildcard_source, set()):
                ...
                if verbose:
                    print(f"DEBUG: Wildcard import: '{symbol}' in {os.path.basename(file_path)} comes from {os.path.basename(wildcard_source)}")
...
def combine_project(main_file, output_file, verbose=False):
    ...
    dep_graph, declared_symbols, symbol_origins, external_imports, scanned_files = analysis_result
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Combine and prefix a multi-file Python project into a single script.')
    parser.add_argument('input_file', help='Path to the main Python file of the project')
